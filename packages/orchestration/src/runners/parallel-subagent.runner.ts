...
export class ParallelSubAgentRunner {
  ...
  private aggregateConsensus(
    ...
    const threshold = this.options.consensusThreshold ?? 0.7;
    const meetsThreshold = consensusScore !== undefined && consensusScore >= threshold;
    ...
    const header = meetsThreshold
      ? `**Consensus Reached** (Score: ${consensusScore!.toFixed(2)}, Threshold: ${threshold})\n\n`
      : '';

    const topAgent = meetsThreshold
      ? successful.reduce((best, r) => ((r.score ?? 0) > (best.score ?? 0) ? r : best)).agentName
      : undefined;

    return {
      combinedResponse: header + agentSummaries,
      results,
      successCount,
      failedCount,
      consensusScore,
