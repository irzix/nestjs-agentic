...
| Human approval | Experimental | Approval and rejection resume the original model turn and use a serializable `PendingApproval` record, so a durable `ApprovalStore` (e.g. `RedisApprovalStore`) can survive a process restart or resolve on a different instance. Settlement is atomic (at most once) via `ApprovalStore.claim()`. Idempotency keys for retrying a claimed-but-failed tool, execution-loop checkpointing independent of the transcript, and approval expiry remain open. |
