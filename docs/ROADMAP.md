...
| Human approval | Experimental | Approval and rejection resume the original model turn and use a serializable `PendingApproval` record, so a durable `ApprovalStore` (e.g. `RedisApprovalStore`) can survive a process restart or resolve on a different instance. Execution-loop checkpointing independent of the transcript, exactly-once guarantees, and approval expiry remain open. |
